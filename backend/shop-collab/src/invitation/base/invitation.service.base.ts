...
import { Prisma, Invitation, User } from "@prisma/client";
...
export class InvitationServiceBase {
  ...
  async getByOwner(parentId: string): Promise<User | null> {
  ...
  async getInvitedUser(parentId: string): Promise<User | null> {
    return this.prisma.invitation
      .findUnique({
        where: { id: parentId },
      })
